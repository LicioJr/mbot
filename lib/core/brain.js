...
function reply(ii, cb) {
    ...
    _nlp.classify(dialog, (er, oo) => {
        ...
        // text
        if (oo.reply)
            return cb(null, {text: oo.reply});

        // dictionary to array
        let aBots = [];
        for (let b in oo.bots)
            aBots.push(oo.bots[b]);
        ...
        // two bots (display helps)
        if (aBots.length == 2) {
            let text = aBots.map(b => b.help).join('\n');
            return cb(null, {text});
        }
        ...
        try {
            // async call
            bot.reply(dialog, function(er, oo) {
                if (er) {
                ...
                // save response
                if (oo) {
                    dbDialog.reply = oo.text;
                    dbDialog.bot = bot.name;
                    _db.model.dialog.createWithUser(dbDialog, function(er, oo) {
                        if (er) log.error(`lib/core/brain.reply (dialog.create): ${er}`);
                        // else log.info('lib/core/brain (dialog.create): ' + util.inspect(oo));
                    });
                }
